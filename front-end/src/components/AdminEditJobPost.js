...
function AdminEditJobPost() {
  ...
  const onSubmit = async (e) => {
    ...
    formData.append("file_name", jobDetails.file_name);
    formData.append("file", jobDetails.file);

    await axios
      .put(`/edit_job_post/${id}`, formData)
      .then((response) => {
  ...
  const loadJobpost = async () => {
    const result = await axios.get(`/get_job_post/${id}`);
            ...
            <div className="grid gap-6 mb-6 md:grid-cols-2">
              ...
              <div>
                <label
                  ...
                  className="block mb-2 text-sm font-medium text-gray-900 dark:text-gray"
                >
                  Open Date: {jobDetails.open_date}
                </label>
                <input
                  type="date"
                  id="open_date"
